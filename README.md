...
# Adding the first REST service

Before a Nobel Prize can be created a Person is necessary.
Person in turn uses a Country but countries are created in the database.
To get a Country I first created
[CountryService.java](src/main/java/com/schotanus/nobel/service/CountryService.java)
and [CountryRepository.java](src/main/java/com/schotanus/nobel/repository/CountryRepository.java).

Looking at Person I noticed an identifier beside the primary key, was missing,
so I added a personIdentifier to the database and model.
The [nobel.yml](src/main/resources/nobel.yml) was also updated to create a person and to select a single person.
In the process I made some fields read-only and added validation to the different properties.
In my [pom.xml](pom.xml) file I set useBeanValidation to true and added a dependency
on quarkus-hibernate-validator.

At this stage I generated the sources again and created
[PersonApiImpl.java](src/main/java/com/schotanus/nobel/api/PersonApiImpl.java),
[PersonService.java](src/main/java/com/schotanus/nobel/service/PersonService.java) and
[PersonRepository.java](src/main/java/com/schotanus/nobel/repository/PersonRepository.java)
and created some request in Bruno to test the code.

## Error handling
In case of an error a stacktrace is produced.
To avoid passing this to the client of my services I implemented a simple mapping
from SQL exception to an error response in
[SqlExceptionMapping.java](src/main/java/com/schotanus/nobel/exception/SqlExceptionMapping.java) 

## Adding unit tests and integration tests
I had to make some adjustments to the [pom.xml](pom.xml) file, to get integration tests to work.
Just look at the plugin containing:
```<goal>integration-test</goal>```

## Adding code coverage
Quarkus can use [Jacoco](https://quarkus.io/guides/tests-with-coverage) for code coverage.
Simply add a dependency to the [pom.xml](pom.xml) file.
By default the coverage of generated source code is included.
To prevent this add the following lines to your application.properties file.
```text
# Jacoco
quarkus.jacoco.title=Nobel Prize
quarkus.jacoco.excludes=**/model/*,**/tables/**/*
```
